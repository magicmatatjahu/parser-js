...
function retrieveMessageId(message: v3.MessageObject): { messageId: string; path: JsonPath } | undefined {
  if (Array.isArray(message.traits)) {
    for (let i = message.traits.length - 1; i >= 0; i--) {
      const trait = message.traits[i] as v3.MessageTraitObject;
...
export const messageIdUniqueness = createRulesetFunction<v3.AsyncAPIObject, null>(
  {
    input: {
      type: 'object',
      properties: {
