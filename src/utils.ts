...
export function createDetailedAsyncAPI(source: string | Record<string, unknown>, parsed: Record<string, unknown>): DetailedAsyncAPI {
  const unparsed: Record<string, unknown> = typeof source === 'string' ? JSON.parse(source) : source;
  return {
    source,
    unparsed,
    parsed,
...
export function getCircularSchema($ref: string, jsonPath: string, data: Record<string, any>): any {
