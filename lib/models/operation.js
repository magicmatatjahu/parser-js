...
class Operation extends OperationTraitable {
   ...
   */
  hasMultipleMessages() {
    if (this._json.message && this._json.message.oneOf && this._json.message.oneOf.length) return true;
    return false;
  }
   ...
   */
  hasSingleMessage() {
    return isNotEmptyObject(this._json.message) && !this._json.message.hasOwnProperty('oneOf');
  }
