...
/**
 * Marks all recursive references as recursive.
 * 
 * @private
 * @param {Any} value 
 * @param {Any} cache 
 */
function markCircularsInDoc(doc) {
  const value = doc.json();
  let cache = new Set();
  const ctx = { previous: null, value, propOrIndex: null };

  cache.add(value);
  for(const idx in value) {
    markCirculars(value[idx], cache, ctx, value, idx);
  }
  cache = null;
}
 ...
 */
let it = 0;
function markCirculars(value, cache, ctx, parent, propOrIndex) {
  if (propOrIndex === 'x-parser-original-ref') {
    return;
  }

  // console.log(propOrIndex)
  // only object and array are references
  if (typeof value == 'object' && value !== null) {
    // if the schema was marked as circular skip the iteration
    if (value[String(xParserCircle)] === true) {
      return;
    }

    it++;
    const originalRef = value['x-parser-original-ref'];
    if (cache.has(value) || cache.has(originalRef)) {
      let originalValue = value;
      if (cache.has(originalRef)) originalValue = originalRef;

      // const map = {};
      const order = [];

      // find the last occurrence of the reference pointed by value
      let parentCtx = ctx;
      while (parentCtx !== null) {
        if (parentCtx.value === originalValue) break;
        order.push(parentCtx.propOrIndex);
        // console.log(parentCtx.value['x-parser-original-ref']);
        // map[parentCtx.propOrIndex] = parentCtx.value['x-parser-original-ref'] || parentCtx.value;
        parentCtx = parentCtx.previous;
      }
      // console.log(order, propOrIndex)

      let parentValue = parentCtx.value;
      const parentOrder = parentValue[order[order.length-1]];
      const makeNewCopy = parentOrder === undefined || parentOrder['x-parser-original-ref'] === undefined;
      // console.log(makeNewCopy)
      ...
      if (makeNewCopy === true) {
        for (let i = order.length - 1; i >= 0; i--) {
          ...
          const newValue = {
            ...parentValue[prop],
            'x-parser-original-ref': parentValue[prop],
          };
        ...
        for (let i = order.length - 1; i >= 0; i--) {
          const prop = order[i];
          if (parentValue[prop]['x-parser-original-ref'] === undefined) {
            const newValue = {
              ...parentValue[prop],
              'x-parser-original-ref': parentValue[prop],
            };
            ...
            // console.log(prop, parentValue[prop])
            parentValue = parentValue[prop];
          }
        }
      }
      // console.log(propOrIndex)
      ...
      // let parentValue = parentCtx.value;
      // let next = parentCtx.next;
      // while (next !== null) {
      //   console.log('next: ', next.propOrIndex)
      //   // console.log(next.value)
      //   if (parentValue[next.propOrIndex]['x-parser-original-ref'] === undefined) {
      //     const newValue = {
      //       ...next.value,
      //       'x-parser-original-ref': next.value,
      //     };
      //     parentValue[next.propOrIndex] = newValue;
      //     parentValue = newValue;
      //   } else {
      //     parentValue = parentValue[next.propOrIndex];
      //   }
      //   if (next.next === null) break;
      //   next = next.next;
      // }

      // console.log('last: ', propOrIndex)

      // if schema is circular then create from given object "new" object with this same shape
      // and point to original reference and mark object as circular
      // console.log(propOrIndex, parentValue, parentValue['x-parser-original-ref'])
      parentValue[propOrIndex] = {
        ...value,
        'x-parser-original-ref': originalValue,
        [String(xParserCircle)]: true,
      };
      return;
    }
    ...
    const newCtx = { previous: ctx, value, propOrIndex };

    cache.add(value);
    // iterate over all properties from object and indexes from array
    for(const idx in value) {
      markCirculars(value[idx], cache, newCtx, value, idx);
    }
    cache.delete(value);
  }
}
