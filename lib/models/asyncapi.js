...
function markCircularsInDoc(doc) {
  ...
  let cache = new Set();
  const ctx = { previous: null, value, propOrIndex: null, next: null };
  ...
  if (typeof value == 'object' && value !== null) {
    ...
    if (cache.has(value)) {
      ...
      while (parentCtx !== null) {
        if (parentCtx.value === value) break;
    ...
    const newCtx = { previous: ctx, value, propOrIndex, next: null };
    ctx.next = newCtx; // tutaj jest problem nadpisywanie contextu
